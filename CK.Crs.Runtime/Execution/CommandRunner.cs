...
namespace CK.Crs.Runtime
{
    class CommandRunner
    {
        public static async Task ExecuteAsync( CommandContext context, ICommandHandlerFactory handlerFactory, ICommandDecoratorFactory commandDecoratorFactory )
        {
            var mon = context.ExecutionContext.Monitor;

            var decorators = context.ExecutionContext.Action.Description.Descriptor.Decorators.Select( commandDecoratorFactory.CreateDecorator ).ToArray();
            using( mon.OpenTrace().Send( $"Running Command [{context.ExecutionContext.Action.Description.Descriptor.CommandType.Name}]..." ) )
            {
                ICommandHandler handler = handlerFactory.CreateHandler( context.ExecutionContext.Action.Description.Descriptor.HandlerType );
